...
public class OpeningBook {
    private final String[] OPENING_BOOK = {
            "C4e3F6e6F5g6E7c5",
            "C4e3F6e6F5g6",
    ...
    private final List<List<PlayerMove>> openingSequences;

    public OpeningBook() {
        openingSequences = new ArrayList<>();
        for (String sequence : OPENING_BOOK) {
            List<PlayerMove> moves = new ArrayList<>();
            boolean isPlayer1 = true;
            for (int i = 0; i < sequence.length(); i += 2) {
                char letter = sequence.toLowerCase().charAt(i);
                int number = Character.getNumericValue(sequence.charAt(i + 1));
                int x = letter - 'a';
                int y = number - 1;
    ...
    public PlayerMove getOpeningMove(List<PlayerMove> gameMoves) {
        for (List<PlayerMove> sequence : openingSequences) {
            if (gameMoves.size() < sequence.size() && gameMoves.equals(sequence.subList(0, gameMoves.size()))) {
                return sequence.get(gameMoves.size());
            }
        }
        return null;
    }
